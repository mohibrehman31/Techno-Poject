...
def predict(request):
    if request.method == 'POST':
        ...
        if cabin == 'B':
            ...
            for j in range(2):
                p_0 = model_time.predict(
                    [[j, dept_dict[dcity], Dept_date, arrival_dict[acity], 0, scale_0, weekday, dept_hours, temp_time]])

            for j in range(2):
                p_1 = model_time.predict(
                    [[j, dept_dict[dcity], Dept_date, arrival_dict[acity], 1, scale_1, weekday, dept_hours, temp_time]])

            sc = load('B/B_price_dept_hours_std_scaler.bin')
            h_0 = sc.transform(p_0.reshape(-1, 1))
            h_1 = sc.transform(p_1.reshape(-1, 1))

            headings = ("Airline", "Stops", "Source_city",
                        "Arrival_city", "Duration", "Optimal_time", "Price")
            data = []

            for j in range(2):
                pred = model_price.predict(
                    [[j, dept_dict[dcity], Dept_date, arrival_dict[acity], 0, scale_0, weekday, h_0, temp_time]])
                data.append([airline_dict[j].capitalize(), 0, dcity.capitalize(
                ), acity.capitalize(), k_0, int(p_0[0]), "₹"+str(int(pred[0]))])

            for j in range(2):
                pred = model_price.predict(
                    [[j, dept_dict[dcity], Dept_date, arrival_dict[acity], 1, scale_1, weekday, h_1, temp_time]])
                data.append([airline_dict[j].capitalize(), 1, dcity.capitalize(
        ...
        if cabin == 'E':
            print("HII_E")
            with open("E_price_predict.pkl", "rb") as f:
                model_price = pickle.load(f)
            with open("E_time_predict.pkl", "rb") as f:
                model_time = pickle.load(f)
            with open("E_airline_dict.pkl", "rb") as f:
                airline_dict = pickle.load(f)
            with open("E_arrival_city_dict.pkl", "rb") as f:
                arrival_dict = pickle.load(f)
            with open("E_dept_city_dict.pkl", "rb") as f:
                dept_dict = pickle.load(f)
            sc = load("E_Dept_date_std_scaler.bin")
            Dept_date = sc.transform(day.reshape(-1, 1))
